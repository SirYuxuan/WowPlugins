...
local function ETcfg()
    local cfg = Core.db.profile.eventTracker
...
local function CreateWeeklyTracker(parent, eventKey)
    ...
    local frame = CreateFrame("Frame", addonName .. "ET_" .. eventKey, parent)
...
-- 主框架构建
--------------------------------------------------------------------------------
function Core:CreateEventTrackerFrame()
    ...
    self.eventTrackerFrame = frame
end

--------------------------------------------------------------------------------
-- 更新追踪器布局
--------------------------------------------------------------------------------
function Core:UpdateEventTrackers()
    if not WorldMapFrame then return end
    self:CreateEventTrackerFrame()

    local frame = self.eventTrackerFrame
    local cfg = ETcfg()
...
-- 更新所有追踪器的数据（定时调用）
--------------------------------------------------------------------------------
function Core:TickEventTrackers()
    if not self.eventTrackerFrame or not self.eventTrackerFrame:IsShown() then return end
...
-- 应用字体设置
--------------------------------------------------------------------------------
function Core:ApplyEventTrackerFonts()
    local cfg = ETcfg()
    ...
    for _, tracker in pairs(trackerPool) do
        if tracker.nameText then
            tracker.nameText:SetFont(STANDARD_TEXT_FONT, size, outline)
        end
        if tracker.timerText then
            tracker.timerText:SetFont(STANDARD_TEXT_FONT, size, outline)
        end
        if tracker.statusText then
            tracker.statusText:SetFont(STANDARD_TEXT_FONT, size, outline)
        end
        if tracker.runningTip then
            tracker.runningTip:SetFont(STANDARD_TEXT_FONT, math.max(9, size - 2), outline)
        end
    end
end
...
-- 设置初始化与应用
--------------------------------------------------------------------------------
function Core:ApplyEventTrackerSettings()
    local cfg = ETcfg()
    ...
    if cfg.enabled then
        if not self.eventTrackerTicker then
            self.eventTrackerTicker = C_Timer.NewTicker(0.5, function()
                if Core.db and Core.db.profile and Core.db.profile.eventTracker
                    and Core.db.profile.eventTracker.enabled then
                    Core:TickEventTrackers()
                end
            end)
        end
                ...
                if EventRegistry then
                    pcall(function()
                        EventRegistry:RegisterCallback("WorldMapMinimized", function()
                            C_Timer.After(0.15, function() Core:UpdateEventTrackers() end)
                        end)
        ...
        if self.eventTrackerFrame then
            self.eventTrackerFrame:Hide()
        end
    end
end
...
-- 可见性控制
--------------------------------------------------------------------------------
function Core:ToggleEventTracker(forceVisible)
    local cfg = ETcfg()
